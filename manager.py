...
def reset_lock_timer(event=None):
    ...
    if lock_timer_id:
        root.after_cancel(lock_timer_id)
    lock_timer_id = root.after(120000, lock_app) # 120,000 ms = 2 minutes
...
def copy_to_clipboard(text, btn_widget):
    ...
    # Feature 2: Clipboard Auto-Clear
...
# --- Feature 3: Strength Meter Logic ---
...
def view_password_details(event):
    ...
    btn_copy_usr = ttk.Button(view_win, text="Copy User", command=lambda: copy_to_clipboard(username, btn_copy_usr))
...
def import_csv():
    file_path = filedialog.askopenfilename(filetypes=[("CSV Files", "*.csv")], title="Select Unencrypted CSV")
    if not file_path: return
    if not verify_master_password(): return
...
# --- Feature 4: Multi-Language Dictionary ---
LANGUAGES = {
    "English": {"t1": "Generator & Save", "t2": "Saved Passwords", "t3": "Backup & Restore", "t4": "About", "gen_btn": "Generate Password", "save_btn": "Encrypt & Save"},
    "Turkish": {"t1": "Oluştur & Kaydet", "t2": "Kayıtlı Şifreler", "t3": "Yedekle & Yükle", "t4": "Hakkında", "gen_btn": "Şifre Oluştur", "save_btn": "Şifrele ve Kaydet"},
    "Polish": {"t1": "Generator i Zapis", "t2": "Zapisane Hasła", "t3": "Kopia zapasowa", "t4": "O aplikacji", "gen_btn": "Generuj Hasło", "save_btn": "Szyfruj i Zapisz"},
    "Russian": {"t1": "Генератор и Сохранение", "t2": "Сохраненные пароли", "t3": "Резервное копирование", "t4": "О программе", "gen_btn": "Сгенерировать пароль", "save_btn": "Зашифровать и сохранить"},
    "German": {"t1": "Generator & Speichern", "t2": "Gespeicherte Passwörter", "t3": "Sichern & Wiederherstellen", "t4": "Über", "gen_btn": "Passwort generieren", "save_btn": "Verschlüsseln & Speichern"}
}
...
def change_language(*args):
    ...
    notebook.tab(0, text=trans["t1"])
    notebook.tab(1, text=trans["t2"])
    notebook.tab(2, text=trans["t3"])
    notebook.tab(3, text=trans["t4"])
    generate_btn.config(text=trans["gen_btn"])
    save_btn.config(text=trans["save_btn"])
...
root.deiconify() 
root.title("Local Password Manager v1.5") 
...
save_btn.grid(row=8, column=0, columnspan=5, pady=5)
for i in range(1, 5): tab_generator.columnconfigure(i, weight=1)

# === TAB 2: VAULT ===
search_frame = ttk.Frame(tab_vault)
search_frame.pack(fill="x", padx=10, pady=10)
ttk.Label(search_frame, text="Search Vault:").pack(side="left")
...
ttk.Label(tab_sync, text="Save a secure .vault file to your PC or a USB drive.", font=("Arial", 10)).pack(anchor="w", pady=(0, 10))
btn_export = ttk.Button(tab_sync, text="Export Vault...", command=export_vault)
btn_export.pack(anchor="w", pady=(0, 20))

ttk.Label(tab_sync, text="Import Backup (.vault)", font=("Arial", 12, "bold")).pack(anchor="w", pady=(0, 5))
ttk.Label(tab_sync, text="Load an encrypted .vault file. Passwords will be instantly merged.", font=("Arial", 10)).pack(anchor="w", pady=(0, 10))
btn_import = ttk.Button(tab_sync, text="Import Vault...", command=import_vault)
btn_import.pack(anchor="w", pady=(0, 20))

ttk.Label(tab_sync, text="Import from CSV (Chrome / LastPass)", font=("Arial", 12, "bold")).pack(anchor="w", pady=(0, 5))
ttk.Label(tab_sync, text="Select an unencrypted CSV file to instantly encrypt into your vault.", font=("Arial", 10)).pack(anchor="w", pady=(0, 10))
...
ttk.Label(tab_about, text="Version 1.5", font=("Arial", 10)).pack(pady=(0, 20))

ttk.Label(tab_about, text="Display Language:", font=("Arial", 10, "bold")).pack(pady=(10, 2))
